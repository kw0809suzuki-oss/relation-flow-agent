...
def agent(obs):
    ...
    if strategy_name not in ("LIQUID","ENDGAME") and land_cost and remaining_days>=9 and occupancy>=strategy["occupancy_target"] and projected_cash-land_cost>=reserve and len(market)<10:
        if not (str((_FLOW_ABSTRACTION or {}).get("phase","") or "").upper()=="OBJECTIVE_PRESSURE_GUIDANCE" and os.getenv("ORIGIN_GUIDANCE_MODE","none").strip().lower() in ("throughput_match","throughput_with_slack") and not land_realizable_ok):
