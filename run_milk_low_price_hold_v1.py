...
def main():
    ...
    for seed,seat in CASES:
        ...
        rows.append({"seed":seed,"seat":seat,"baseline_self":b,"hold_self":h,"self_diff":h-b,
                     "baseline_margin":bm,"hold_margin":hm,"margin_diff":hm-bm})
    mean=lambda k:sum(r[k] for r in rows)/len(rows)
    summary={"case_count":len(rows),"mean_baseline_self":mean("baseline_self"),"mean_hold_self":mean("hold_self"),
