#!/usr/bin/env python3
"""Fresh-12 paired evaluation: Full v1 vs Whole v2.5 relation gate."""
...
import full_strong_origin_v2_5_relation_gate as candidate_agent

OPPONENT = "opponents/seyamalam_v21.py"
CASES = tuple((seed, (seed - 3786) % 2) for seed in range(3786, 3798))
...
def main():
    ...
    totals = Counter()
    for row in rows:
        for key, value in row["candidate_telemetry"].get("whole_v2_5", {}).items():
    ...
    summary = {
        ...
        "whole_v2_5_totals": dict(totals),
    }

    result = {
        "schema": "kaggriculture.full-v2-5-relation-gate-pair.v1",
        "question": "can current relation gating retain self-score gain while improving relative outcome?",
        "baseline": "full_strong_origin_v1.py",
        "candidate": "full_strong_origin_v2_5_relation_gate.py",
        "cases": rows,
        "summary": summary,
        "runtime_inputs_excluded": ["seed", "paired_difference", "terminal_reward", "future_state"],
        "baseline_mutated": False,
        "causal_attribution": False,
        "evaluation_policy": "external short-loop score probe; terminal judgment first",
    }
    Path("full_v2_5_relation_gate_pair_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n")
    print("FULL_V2_5_RELATION_GATE_PAIR " + json.dumps(summary, separators=(",", ":")))
