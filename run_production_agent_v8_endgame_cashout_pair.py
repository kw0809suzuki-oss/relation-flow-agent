#!/usr/bin/env python3
"""Fresh10 paired evaluation: v8 endgame cash-out vs v6 Bridge1.
...
import production_agent_v8_endgame_cashout as candidate_agent

OPPONENT="opponents/seyamalam_v21.py"
CASES=tuple((seed,(seed-4102)%2) for seed in range(4102,4112))
...
def main():
    ...
    out={"schema":"kaggriculture.production-v8-endgame-cashout-pair.v1","baseline":"production_agent_v6_bridge1.py","candidate":"production_agent_v8_endgame_cashout.py","design_unit":"Day24+: preserve SELL and work actions; remove new market investment","observation_boundary":{"bundle_comparison_only":True,"bundle_semantics_added":False,"bundle_used_for_control":False,"terminal_applied_after_observation":True,"causal_attribution":False},"cases":rows,"summary":summary}
    Path("production_agent_v8_endgame_cashout_pair_result.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
    print("PRODUCTION_V8_ENDGAME_CASHOUT_PAIR "+json.dumps(summary,separators=(",",":")))
