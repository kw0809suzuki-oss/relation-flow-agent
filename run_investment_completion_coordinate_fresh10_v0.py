#!/usr/bin/env python3
"""Paired fresh10 Battle for Investment Completion Coordinate v0."""
import json, os
...
OUT=Path(f"investment_completion_coordinate_v0_{SEED}.json")
...
def main():
    ...
    payload={
        "schema":"kaggriculture.investment-completion-coordinate.v0",
        "seed":SEED,"seat":SEAT,
        "baseline":baseline,
        "candidate":variant,
        "reachability":{
            "seed_considered_after_d14":tele.get("seed_considered_after_d14",0),
            "seed_allowed_completion":tele.get("seed_allowed_completion",0),
            "seed_removed_completion":tele.get("seed_removed_completion",0),
            "land_removed_d14":tele.get("land_removed_d14",0),
            "cow_removed_d14":tele.get("cow_removed_d14",0),
            "reached":tele.get("seed_allowed_completion",0)>0 or tele.get("seed_removed_completion",0)>0,
            "allow_days":sorted({e["day"] for e in tele.get("events",[]) if e.get("kind")=="allow"}),
            "remove_days":sorted({e["day"] for e in tele.get("events",[]) if e.get("reason")=="full_cycle_incomplete"}),
        },
        "flow_change":flow,
        "terminal_effect":{
        ...
        "boundary":[
            ...
            "Candidate changes only post-D14 SEED closure to a minimum full Cash Conversion Cycle completion test.",
            "LAND and COW remain fixed D14 closure.",
            "Seed completion uses the native harvest timing: WHEAT/MELON MAX_YIELD_DAY, STRAWBERRY FIRST_YIELD.",
            "Completion lower bound includes Plant + Grow + Harvest + shortest current empty-tile-to-shed transport + DROP + SELL.",
            "This tests completion only, not expected profit.",
            "Fresh paired seed/seat comparison; no auto-adoption."
        ]
    }
    OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
    print("INVESTMENT_COMPLETION_COORDINATE_RESULT "+json.dumps(payload,ensure_ascii=False,separators=(",",":")))
