#!/usr/bin/env python3
"""Fresh-50 paired evaluation: Full v1 vs Whole v2.1 zero-detour."""
...
def main():
    ...
    totals = Counter()
    for row in rows:
        for key, value in row["candidate_telemetry"].get("whole_v2_1", {}).items():
    ...
    summary = {
        ...
        "whole_v2_1_totals": dict(totals),
    }

    result = {
        "schema": "kaggriculture.full-v2-1-zero-detour-pair.v1",
        "question": "can a zero-detour edge intervention fire often enough to matter without disrupting Full v1 productive hand flow?",
        "baseline": "full_strong_origin_v1.py",
        "candidate": "full_strong_origin_v2_1_zero_detour.py",
        "cases": rows,
        "summary": summary,
        "runtime_inputs_excluded": ["seed", "paired_difference", "terminal_reward", "future_state"],
        "baseline_mutated": False,
        "candidate_change_unit": "feed-first guard + PASS/movement-only zero-detour fertilizer edge intervention",
        "result_used_to_define_candidate": False,
        "causal_attribution": False,
        "evaluation_policy": "self score first; downside/stability second; margin/win secondary",
    }
    Path("full_v2_1_zero_detour_pair_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n")
    print("FULL_V2_1_ZERO_DETOUR_PAIR " + json.dumps(summary, separators=(",", ":")))
