"""Model-generated Option Preservation interpretation bundle v0.

This body is derived from the frozen Strong Origin, but the added policy is not
a Flow-chan rule. Flow-chan only transmits a semantic abstraction. The model
interprets that abstraction as a candidate policy and Battle evaluates it.

Interpretation hypothesis:
Do not commit cash into more crop-specific seed inventory than can be converted
into production through the currently available near-term planting surface.
This is a generated candidate, not evidence that the abstraction is correct.
"""
...
def agent(obs):
    ...
    option_preservation_received = abstraction_phase == "OPTION_PRESERVATION"
    interpretation_mode = os.getenv("ORIGIN_ABSTRACTION_INTERPRETATION", "capacity_aligned").strip().lower()
    ...
    new_seed_commitment_budget = max(0, immediate_planting_surface - existing_seed_commitment)
    alternative_cash_floor = 0
    staged_commitment_cap = 1

    for _,crop,buy in sorted(seed_plan,reverse=True):
        if len(market)>=10: break

        if option_preservation_received and interpretation_mode == "capacity_aligned":
            buy=min(buy,new_seed_commitment_budget)

        elif option_preservation_received and interpretation_mode == "alternative_affordability":
            other_viable = [
                c for c in BASE_PRICE
                if c != crop
                and scores.get(c, 0) > 0
                and remaining_days > FIRST_YIELD[c] + 1
                and targets[c] > len(my_plants[c])
            ]
            alternative_cash_floor = sum(SEED_COST[c] for c in other_viable)
            affordable_with_options=max(
                0,
                int((projected_cash-reserve-alternative_cash_floor)//SEED_COST[crop])
            )
            buy=min(buy,affordable_with_options)

        elif option_preservation_received and interpretation_mode == "staged_commitment":
            buy=min(buy,staged_commitment_cap)
        ...
        if projected_cash-cost>=reserve:
            ...
            if option_preservation_received and interpretation_mode == "capacity_aligned":
                new_seed_commitment_budget=max(0,new_seed_commitment_budget-buy)
