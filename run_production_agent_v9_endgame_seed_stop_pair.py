#!/usr/bin/env python3
"""Fresh10: v9 endgame seed-stop vs v6 Bridge1.
Bundle comparison first; terminal applied after observation.
"""
...
import production_agent_v9_endgame_seed_stop as candidate_agent

OPPONENT="opponents/seyamalam_v21.py"
CASES=tuple((seed,(seed-4112)%2) for seed in range(4112,4122))
...
def classify(action,allocation):
...
def play(seed,seat,module):
    module.reset_telemetry(); allocation=Counter(); days={}
    def wrapped(obs):
        day=int(obs.get("day",0)); rec=days.setdefault(day,{})
        rec["self"]=public_bundle(obs,seat); rec["opponent"]=public_bundle(obs,1-seat)
        action=module.agent(obs); classify(action,allocation); return action
    env=make("kaggriculture",configuration={"seed":seed},debug=False)
    players=[OPPONENT,OPPONENT]; players[seat]=wrapped; env.run(players)
    rewards=[state.reward for state in env.state]
    return {"terminal":score(rewards,seat),"allocation":dict(allocation),"bundle":[{"day":d,**days[d]} for d in sorted(days)],"telemetry":copy.deepcopy(module.get_telemetry())}
...
def main():
    ...
    for seed,seat in CASES:
        ...
        rows.append({"seed":seed,"seat":seat,"baseline":base,"candidate":cand,
            ...
            "opponent_delta":cand["terminal"]["opponent"]-base["terminal"]["opponent"],
            "margin_delta":cand["terminal"]["margin"]-base["terminal"]["margin"],
            "investment_delta":cand["allocation"].get("investment",0)-base["allocation"].get("investment",0),
    ...
    summary={"cases":len(rows),"self_delta":stats("self_delta"),"opponent_delta":stats("opponent_delta"),"margin_delta":stats("margin_delta"),"investment_delta":stats("investment_delta"),"maintenance_delta":stats("maintenance_delta"),"production_delta":stats("production_delta"),"realization_delta":stats("realization_delta"),"baseline_wins":sum(r["baseline"]["terminal"]["win"] for r in rows),"candidate_wins":sum(r["candidate"]["terminal"]["win"] for r in rows)}
    out={"schema":"kaggriculture.production-v9-endgame-seed-stop-pair.v1","baseline":"production_agent_v6_bridge1.py","candidate":"production_agent_v9_endgame_seed_stop.py","design_unit":"Day24+: suppress BUY_SEED only; keep all other v6 behavior","observation_boundary":{"bundle_comparison_only":True,"bundle_semantics_added":False,"bundle_used_for_control":False,"terminal_applied_after_observation":True,"causal_attribution":False},"cases":rows,"summary":summary}
    Path("production_agent_v9_endgame_seed_stop_pair_result.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
    print("PRODUCTION_V9_ENDGAME_SEED_STOP_PAIR "+json.dumps(summary,separators=(",",":")))
