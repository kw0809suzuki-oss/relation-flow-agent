#!/usr/bin/env python3
"""Resource conversion path observer v1.

Desk coordinate: Remaining Time x Value Flow.
Inspect Day25-29 non-overlapping cycles and trace product composition across
the 6 turns before SELL to see whether strong throughput is associated with
specific resource conversion paths (e.g. WHEAT -> MILK -> Money).
...
def play(seed,seat):
    ...
    def observed(obs):
        ...
        trace.append({
          "turn":turn,"day":int(obs.get("day",0)),"money":money,
          "stock_qty":ps["qty"],"stock_value":ps["value"],"prices":ps["prices"],
    ...
    while i<len(trace):
        ...
        def q(row,k): return float(row["stock_qty"].get(k,0))
        conversion={
          k: q(srow,k)-q(pre[0],k) for k in PRODUCTS
        }
        sell_by=Counter()
        for a in srow["sells"]:
        ...
        cycles.append({
          ...
          "stock_value_start":pre[0]["stock_value"],
          "stock_value_presell":srow["stock_value"],
          "delta_WHEAT":conversion["WHEAT"],"delta_MILK":conversion["MILK"],
          "delta_FERTILIZER":conversion["FERTILIZER"],
          "sell_WHEAT":sell_by.get("WHEAT",0),"sell_MILK":sell_by.get("MILK",0),
          "trajectory":[{"turn":x["turn"],"WHEAT":q(x,"WHEAT"),"MILK":q(x,"MILK"),
                         "FERTILIZER":q(x,"FERTILIZER"),"stock_value":x["stock_value"],"money":x["money"]}
                        for x in pre]
        })
        i=end+1
...
def band(xs):
    return {
      ...
      "mean_stock_value_start":mean([c["stock_value_start"] for c in xs]),
      "mean_stock_value_presell":mean([c["stock_value_presell"] for c in xs]),
      "mean_delta_WHEAT":mean([c["delta_WHEAT"] for c in xs]),
      "mean_delta_MILK":mean([c["delta_MILK"] for c in xs]),
      "mean_delta_FERTILIZER":mean([c["delta_FERTILIZER"] for c in xs]),
      "mean_sell_WHEAT":mean([c["sell_WHEAT"] for c in xs]),
      "mean_sell_MILK":mean([c["sell_MILK"] for c in xs]),
    }
...
def main():
    ...
    out={
      "schema":"resource-conversion-path.v1","objective":"terminal self",
      "coordinate":"Remaining Time x Value Flow",
      "window":"Day25-29; six turns before SELL",
      "question":"which product-composition changes precede strong late throughput?",
      "policy_mutated":False,"causal_attribution":False,
      "low_terminal":summarize(ranked[:n]),"high_terminal":summarize(ranked[-n:]),
      "boundary":"delta quantities are observed stock changes, not proven physical conversion or causal production."
    }
    open("resource_conversion_path_v1.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
    print("RESOURCE_CONVERSION_PATH_V1 "+json.dumps({
