...
def agent(obs):
    ...
    def choose_crop(seed_allowance):
        ...
        return None
    # Candidate v0: Day-0 unit target reservation only.
    # The native crop targets/scores are unchanged.  On Day 0, once an earlier
    # unit claims a work target, later units do not claim the same tile.
    ...
    def reserve(target):
        if day == 0 and target is not None:
            reserved_work_targets.add(tuple(target))

    def unit_action(x,y,seeds):
        cur=tiles[y][x]
        here=(x,y)
        here_available=(day != 0 or here not in reserved_work_targets)
