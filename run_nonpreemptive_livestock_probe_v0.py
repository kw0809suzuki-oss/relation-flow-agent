#!/usr/bin/env python3
"""Paired Non-preemptive Livestock Probe v0.
...
import strong_origin_v2_nonpreemptive_livestock_v0 as candidate

SEED=int(os.environ["BATTLE_SEED"])
SEAT=int(os.environ["BATTLE_SEAT"])
OUT=Path(f"nonpreemptive_livestock_probe_v0_{SEED}.json")
...
payload={
 ...
 "boundary":[
   "Candidate changes only livestock-vs-base unit-action arbitration.",
   "No PLANT or crop action is forced.",
   "New livestock work may use only PASS base units; already-engaged livestock work may continue.",
   "Market livestock rules, crop targets, COW targets, HIRE, LAND and D14 closure are unchanged.",
   "No adoption is automatic."
 ]
}
OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
print("NONPREEMPTIVE_LIVESTOCK_PAIR "+json.dumps({"seed":SEED,"delta":payload["delta"],"baseline_terminal":b["terminal"],"candidate_terminal":c["terminal"],"baseline_commitment":b["commitment"],"candidate_commitment":c["commitment"]},ensure_ascii=False,separators=(",",":")))
