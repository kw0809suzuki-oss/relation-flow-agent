"""Direction-guided Strong Origin experimental body v0.

Derived from the frozen Strong Origin. The only added capability is that the
model's own previously selected BUY_SEED crop can be consulted while generating
seed-purchase and planting candidates. Flow-chan does not choose the crop.
No score, target, purchase quantity, or candidate set is rewritten.
"""

from collections import Counter
from x_engine import XField, choose_x_origin, counter_crop_weights, counter_opportunity, counter_weight

_MODEL_SELECTED_CROP = None
_MODEL_DIRECTION = None


def set_model_selection(selection):
    global _MODEL_SELECTED_CROP
    chosen = dict(selection or {}).get("selected_candidate") if isinstance(selection, dict) else None
    if isinstance(chosen, (list, tuple)) and len(chosen) >= 2 and chosen[0] == "BUY_SEED":
        _MODEL_SELECTED_CROP = str(chosen[1])
    else:
        _MODEL_SELECTED_CROP = None


def get_model_selected_crop():
    return _MODEL_SELECTED_CROP


def set_model_direction(direction):
    global _MODEL_DIRECTION
    _MODEL_DIRECTION = dict(direction or {}) if isinstance(direction, dict) else None


def get_model_direction():
    return dict(_MODEL_DIRECTION or {})
...
def agent(obs):
    ...
    if cweight > 0.0 and scores[best_crop] > 0.0:
        ...
        for donor in sorted(donors,key=lambda c:scores[c]):
            take=min(shift,targets[donor]); targets[donor]-=take; targets[best_crop]+=take; shift-=take
            if shift<=0: break
    # Directional State Read v0: reinterpret the same occupancy budget through
    # the model's own selected direction. This does not increase total target
    # occupancy and does not name a crop externally.
    direction_crop = (_MODEL_DIRECTION or {}).get("selected_crop")
    direction_applied = False
    native_targets = dict(targets)
    if direction_crop in targets and (_MODEL_DIRECTION or {}).get("direction_type") == "selected_crop_state_transition":
        blockers = set((_MODEL_DIRECTION or {}).get("blockers", []) or [])
        if "insufficient_remaining_days" not in blockers:
            donors = [c for c in targets if c != direction_crop and targets[c] > 0]
            if donors:
                donor = min(donors, key=lambda c: (scores.get(c, 0.0), targets[c], c))
                targets[donor] -= 1
                targets[direction_crop] += 1
    ...
    ordered_seed_plan = sorted(
        seed_plan,
        key=lambda row: (row[1] == _MODEL_SELECTED_CROP, row[0], row[1]),
        reverse=True,
    )
    for _,crop,buy in ordered_seed_plan:
    ...
    priority=sorted(
        BASE_PRICE,
        key=lambda c: (c == _MODEL_SELECTED_CROP, deficits[c] > 0, scores[c]),
        reverse=True,
    )
