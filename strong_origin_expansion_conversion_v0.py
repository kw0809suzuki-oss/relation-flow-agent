"""Day0 Target Reservation Candidate v0, copied from frozen Strong Origin.\n\nSingle change: reserve unit work targets on Day 0 to prevent multiple units\nfrom claiming the same tile in one decision pass. All crop targets, scores,\nmarket logic, livestock logic, and post-Day0 behavior remain unchanged.\n\nSource baseline: frozen Strong Origin commit 269edcc96c52b389f65b61808e9bb075b3eea19b.

This file is intentionally frozen. Later experimental bodies wrap this file
...
def agent(obs):
    ...
    def choose_crop(seed_allowance):
        ...
        return None
    # Expansion Conversion v0: coordinate crop work only after LAND has
    # actually expanded the farm.  The expansion happens in market processing
    # after unit actions, so this turns on from the first post-Expansion State.
    conversion_active = quadrants >= 2
    reserved_work_targets=set()

    def nearest_available(points,x,y):
        if not conversion_active:
            return nearest(points,x,y)
        available=[p for p in points if p not in reserved_work_targets]
        return nearest(available,x,y)

    def reserve(target):
        if conversion_active and target is not None:
            reserved_work_targets.add(tuple(target))

    def unit_action(x,y,seeds):
        cur=tiles[y][x]
        here=(x,y)
        here_available=(not conversion_active or here not in reserved_work_targets)
        if here_available and isinstance(cur,dict):
            if cur.get("kind")=="WEED":
                reserve(here); return ["DIG"]
            if cur.get("kind")=="PLANT":
                ...
                if cur.get("yield_units",0)>0 and (crop=="STRAWBERRY" or (crop in MAX_YIELD_DAY and age>=MAX_YIELD_DAY[crop])):
                    reserve(here); return ["HARVEST"]
                if not cur.get("watered_today",False):
                    reserve(here); return ["WATER"]
        if here_available and cur is None:
            crop=choose_crop(seeds)
            if crop:
                seeds[crop]-=1; reserve(here); return ["PLANT",crop]
        for points in (harvest_targets,water_targets,weeds):
            t=nearest_available(points,x,y)
            if t is not None:
                reserve(t); return move(t,x,y)
        if empty_tiles and any(len(my_plants[c])<targets[c] for c in targets):
            t=nearest_available(empty_tiles,x,y)
            if t is not None:
                reserve(t); return move(t,x,y)
        return ["PASS"]
