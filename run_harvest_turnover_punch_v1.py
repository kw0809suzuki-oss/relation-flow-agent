...
import harvest_turnover_punch_v1 as punch
...
def main():
    ...
    out={"hypothesis":"shorten crop production turnover","policy_change":"when a worker is already standing on a plant with yield_units > 0, harvest immediately; otherwise preserve base action","cases":rows,"summary":{"case_count":len(rows),"mean_baseline_self":mean("baseline_self"),"mean_punch_self":mean("punch_self"),"mean_self_diff":mean("self_diff"),"mean_margin_diff":mean("margin_diff"),"improved":sum(x["self_diff"]>0 for x in rows),"worsened":sum(x["self_diff"]<0 for x in rows),"equal":sum(x["self_diff"]==0 for x in rows)}}
    open("harvest_turnover_punch_v1.json","w",encoding="utf-8").write(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
    print("HARVEST_TURNOVER_PUNCH_V1 "+json.dumps(out["summary"],separators=(",",":")))
