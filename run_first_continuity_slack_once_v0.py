...
OUT=Path(f"first_continuity_slack_once_v0_{SEED}.json")
...
def play(mode):
    ...
    for s in snaps(combat.get_trace()):
        ...
        rows.append({
          ...
          "triggered":bool(i.get("first_continuity_slack_triggered",False)),
          "action":s.get("action")
        })
...
def main():
    ...
    once=play("first_continuity_slack_once")
    trigger_rows=[r for r in once["rows"] if r["triggered"]]
    cby={r["turn"]:r for r in current["rows"]}
    action_changed=sum(1 for r in once["rows"] if r["turn"] in cby and cby[r["turn"]]["action"]!=r["action"])
    payload={
      "schema":"kaggriculture.first-continuity-slack-once.v0",
      "seed":SEED,"seat":SEAT,
      "current":{"self":current["self"],"margin":current["margin"]},
      "coarse":{"self":coarse["self"],"margin":coarse["margin"]},
      "one_shot":{"self":once["self"],"margin":once["margin"]},
      "one_shot_vs_current":diff(current,once),
      "one_shot_vs_coarse":diff(coarse,once),
      "trigger_count":len(trigger_rows),
      "trigger_turns":[r["turn"] for r in trigger_rows],
      "action_changed_turns_vs_current":action_changed,
