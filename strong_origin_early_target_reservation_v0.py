"""Early Target Reservation Candidate v0, copied from frozen Strong Origin.\n\nSingle change: reserve unit work targets on Day 0 through Day 4 to prevent multiple units\nfrom claiming the same tile in one decision pass. All crop targets, scores,\nmarket logic, livestock logic, and post-Day4 behavior remain unchanged.\n\nSource baseline: frozen Strong Origin commit 269edcc96c52b389f65b61808e9bb075b3eea19b.
...
def agent(obs):
    ...
    def choose_crop(seed_allowance):
        ...
        return None
    # Candidate v0: Day0-4 unit target reservation only.
    # The native crop targets/scores are unchanged.  On Day 0, once an earlier
    # unit claims a work target, later units do not claim the same tile during the initial window.
    ...
    def reserve(target):
        if day <= 4 and target is not None:
            reserved_work_targets.add(tuple(target))

    def unit_action(x,y,seeds):
        cur=tiles[y][x]
        here=(x,y)
        here_available=(day > 4 or here not in reserved_work_targets)
