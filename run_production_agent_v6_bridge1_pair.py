    ...
    for row in farm.get("tiles", []) or []:
        for tile in row:
            ...
            if isinstance(tile, dict):
                ...
                if tile.get("crop") or tile.get("plant") or tile.get("kind") == "PLANT":
                    planted += 1
    return {
...
def classify(action, allocation, detail):
    ...
    for a in units:
        ...
        if k in MAINT:
            allocation["maintenance"] += 1
        elif k in PROD:
            allocation["production"] += 1
    for o in action.get("market", []) or []:
        if not o: continue
        k = str(o[0]); detail[f"MKT_{k}"] += 1
        if k in {"BUY_LAND","HIRE","BUY_ANIMAL","BUY_PRODUCT","BUY_SEED"}:
            allocation["investment"] += 1
        elif k == "SELL":
            allocation["realization"] += 1


def play(seed, seat, module):
    module.reset_telemetry()
    allocation = Counter(); detail = Counter(); days = {}
    def wrapped(obs):
        day = int(obs.get("day", 0))
        rec = days.setdefault(day, {})
        rec["self"] = public_bundle(obs, seat)
        rec["opponent"] = public_bundle(obs, 1-seat)
        action = module.agent(obs)
        classify(action, allocation, detail)
        return action
    ...
    return {
        ...
        "detail": dict(detail),
        "bundle": [{"day": d, **days[d]} for d in sorted(days)],
        "telemetry": copy.deepcopy(module.get_telemetry()),
    }
...
def main():
    ...
    summary={
        ...
        "maintenance_delta":stats("maintenance_delta"),
        "production_delta":stats("production_delta"),
        "realization_delta":stats("realization_delta"),
        "baseline_wins":sum(r["baseline"]["terminal"]["win"] for r in rows),
        "candidate_wins":sum(r["candidate"]["terminal"]["win"] for r in rows),
    }
    out={
        "schema":"kaggriculture.production-v6-bridge1-pair.v1",
        "baseline":"production_agent_v3_expansion_bridge.py",
        "candidate":"production_agent_v6_bridge1.py",
        "design_unit":"Expansion Bridge dedicated workers 2 -> 1",
        "observation_boundary":{
            "bundle_comparison_only":True,
            "bundle_semantics_added":False,
            "bundle_used_for_control":False,
            "terminal_applied_after_observation":True,
            "causal_attribution":False,
        },
        "cases":rows,
        "summary":summary,
    }
    Path("production_agent_v6_bridge1_pair_result.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n")
    print("PRODUCTION_V6_BRIDGE1_PAIR "+json.dumps(summary,separators=(",",":")))
