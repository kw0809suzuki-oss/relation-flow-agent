"""Public G5 candidate: frozen G4 Strong Origin + coordinated task assignment.

Source lineage:
- G4 Strong Origin / soft COUNTER component from strong_origin.py
- G5 movement-waste reduction ported from private flow-hand commit
  4aa46157ab5e7ec6c3fddc407549ea965f7f72e3

The frozen strong_origin.py remains untouched. This file exists only for paired
terminal evaluation against that baseline.
"""
...
def agent(obs):
    ...
    my_plants = {c: [] for c in BASE_PRICE}
    opp_plants = {c: 0 for c in BASE_PRICE}
    harvest_targets, water_targets, weeds, empty_tiles = [], [], [], []
    unlocked_count = 0
        ...
        for x, tile in enumerate(row):
            ...
            if kind == "WEED":
                ...
                if crop in my_plants:
                    my_plants[crop].append((x, y))
                    age = day - tile.get("planted_day", day)
                    if tile.get("yield_units", 0) > 0 and (crop == "STRAWBERRY" or age >= MAX_YIELD_DAY[crop]):
    ...
    capacity = unlocked_count
    occupied = capacity - len(empty_tiles)
    occupancy = occupied / capacity if capacity else 1.
    field = XField(
        day=day,
        remaining_days=remaining_days,
        my_money=me.get("money", 0),
        opp_money=opp.get("money", 0),
        my_land=len(me.get("unlocked_quadrants", [])),
        opp_land=len(opp.get("unlocked_quadrants", [])),
        my_hands=len(me.get("hands", [])),
        opp_hands=len(opp.get("hands", [])),
        prices=prices,
        my_supply={c: len(my_plants[c]) for c in BASE_PRICE},
        opp_supply=opp_plants,
    )
    xdecision = choose_x_origin(field, BASE_PRICE)
    strategy_name = xdecision.origin
    ...
    for crop in BASE_PRICE:
        if remaining_days <= FIRST_YIELD[crop] + 1:
            scores[crop] = 0.
    best_crop = max(scores, key=scores.get)
    opportunity = counter_opportunity(field, BASE_PRICE)
    cweight = counter_weight(field, BASE_PRICE)

    _TELEMETRY["turns"] += 1
    _TELEMETRY["origins"][strategy_name] += 1
    _TELEMETRY["distortion_sum"] += xdecision.distortion
    _TELEMETRY["distortion_max"] = max(_TELEMETRY["distortion_max"], xdecision.distortion)
    _TELEMETRY["counter_opportunity_sum"] += opportunity
    _TELEMETRY["counter_opportunity_max"] = max(_TELEMETRY["counter_opportunity_max"], opportunity)
    _TELEMETRY["counter_weight_sum"] += cweight
    _TELEMETRY["counter_weight_max"] = max(_TELEMETRY["counter_weight_max"], cweight)
    if xdecision.distortion >= .10:
        _TELEMETRY["distortion_trigger_turns"] += 1
    if scores[best_crop] >= 1.20:
        _TELEMETRY["crop_trigger_turns"] += 1
    if cweight > 0.0:
        _TELEMETRY["counter_active_turns"] += 1

    if xdecision.distortion >= .10 or scores[best_crop] >= 1.20:
        shift = max(2, int(capacity * .12))
        donors = [c for c in targets if c != best_crop and targets[c] > 0]
        for donor in sorted(donors, key=lambda c: scores[c]):
            take = min(shift, targets[donor])
            targets[donor] -= take
            targets[best_crop] += take
            shift -= take
            if shift <= 0:
                break
    if cweight > 0.0 and scores[best_crop] > 0.0:
        shift = max(1, int(capacity * .12 * cweight))
        donors = [c for c in targets if c != best_crop and targets[c] > 0]
        for donor in sorted(donors, key=lambda c: scores[c]):
            take = min(shift, targets[donor])
            targets[donor] -= take
    ...
    if strategy_name not in ("LIQUID", "ENDGAME") and land_cost and remaining_days >= 9 and occupancy >= strategy["occupancy_target"] and projected_cash - land_cost >= reserve and len(market) < 10:
        ...
        projected_cash -= land_cost

    seed_plan = []
    for crop in BASE_PRICE:
        have = private.get("seeds", {}).get(crop, 0)
        live = len(my_plants[crop])
        need = max(0, targets[crop] - live - have)
        if need <= 0 or scores[crop] <= 0:
            continue
        affordable = max(0, int((projected_cash - reserve) // SEED_COST[crop]))
        buy = min(need, affordable, 8)
        if buy > 0:
            seed_plan.append((scores[crop], crop, buy))
    ...
    desired = 1 + min(strategy["max_units"] - 1, max(1, (work + 7) // 8))
    current = 1 + len(me.get("hands", []))
    hires = me.get("hires_today", 0)
    while current < desired and len(market) < 10:
        cost = fib_hire_cost(hires)
        value = 60 if strategy_name == "CROP_RUSH" else 50
        if cost > value or projected_cash - cost < reserve:
            break
        market.append(["HIRE"])
        projected_cash -= cost
        hires += 1
    ...
    # G5: reserve each visible task once per turn so units do not converge on
    # the same target. This is the only deliberate difference from G4.
    reserved = set()
    ...
    def choose_crop(seeds):
        for c in priority:
            if scores[c] > 0 and len(my_plants[c]) < targets[c] and seeds.get(c, 0) > 0:
                return c
        return None
    ...
    def unit_action(x, y, seeds):
        ...
        if isinstance(cur, dict):
            if cur.get("kind") == "WEED":
                reserved.add((x, y))
                return ["DIG"]
            if cur.get("kind") == "PLANT":
                crop = cur.get("crop")
                age = day - cur.get("planted_day", day)
                if cur.get("yield_units", 0) > 0 and (crop == "STRAWBERRY" or (crop in MAX_YIELD_DAY and age >= MAX_YIELD_DAY[crop])):
                    reserved.add((x, y))
                    return ["HARVEST"]
    ...
    units = [("farmer", me["farmer"])] + [(i, p) for i, p in enumerate(me.get("hands", []))]
    actions = {}

    def task_distance(pos):
        x, y = pos
        pts = harvest_targets + water_targets + weeds + empty_tiles
        return min((abs(px - x) + abs(py - y) for px, py in pts), default=999)

    for key, (x, y) in sorted(units, key=lambda u: task_distance(u[1])):
        actions[key] = unit_action(x, y, seeds)

    farmer_action = actions["farmer"]
    hand_actions = [actions[i] for i in range(len(me.get("hands", [])))]
