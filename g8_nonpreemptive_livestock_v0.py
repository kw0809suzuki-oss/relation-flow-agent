"""Non-preemptive livestock overlay probe v0.

Same G8 market/livestock mechanics, but starting a new livestock task may only
use a unit whose Strong Origin base action is PASS. Units already carrying
livestock/output/feed may continue the task so work is not stranded.

This tests arbitration, not forced crop commitment.
"""
...
def agent(obs):
    ...
    def base_for(idx):
        ...
        return hand_actions[idx - 1]

    def engaged_livestock(idx):
        inv = inventories[idx] if idx < len(inventories) else {}
        if inv.get("COW", 0) > 0:
            return True
        if sum(inv.get(k, 0) for k in ("MILK", "WOOL", "EGG", "FERTILIZER")) > 0:
            return True
        if inv.get("WHEAT", 0) > 0 and animal_tiles:
            return True
        x, y = unit_positions[idx]
        tile = tiles[y][x]
        return isinstance(tile, dict) and tile.get("animal") == "COW"

    # Arbitration change only:
    # - never start a new livestock task by replacing an active base action;
    # - continue livestock work already carried by the unit.
    # This removes forced lane capture without forcing PLANT or any crop action.
    for idx in range(lane):
        base = base_for(idx)
        base_op = base[0] if isinstance(base, list) and base else "PASS"
        if base_op != "PASS" and not engaged_livestock(idx):
            continue
        override = livestock_action(idx, unit_positions[idx])
        if override is None:
            continue
        if idx == 0:
            farmer_action = override
        else:
            hand_actions[idx - 1] = override
