...
files=sorted(root.glob("**/nonpreemptive_livestock_probe_v0_*.json"))
if not files:
    files=[Path(p) for p in glob.glob(str(root/"**"/"nonpreemptive_livestock_probe_v0_*.json"),recursive=True)]
...
out={
 "schema":"kaggriculture.strong-origin-v2.nonpreemptive-livestock-probe.fresh5.v0",
 ...
 "boundary":[
   ...
   "Commitment metrics test whether removing forced livestock preemption restores allocation-to-commitment conversion.",
   "No post-hoc rescue condition is added."
 ]
}
Path("nonpreemptive_livestock_probe_v0_fresh5.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
print("NONPREEMPTIVE_LIVESTOCK_FRESH5 "+json.dumps({k:v for k,v in out.items() if k!="cases"},ensure_ascii=False,separators=(",",":")))
