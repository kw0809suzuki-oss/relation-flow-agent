"""Model-side conversion-path-capacity hypothesis bundle v0.

Flow-chan supplies only the abstract direction:
commitment should follow the effective capacity of the path that converts
resources into value.

The model operationalizes structural hypotheses over the native plan.
These are probes, not rules or evidence.
"""
...
def agent(obs):
    ...
    if strategy_name not in ("LIQUID","ENDGAME") and land_cost and remaining_days>=9 and occupancy>=strategy["occupancy_target"] and projected_cash-land_cost>=reserve and len(market)<10:
        if not (str((_FLOW_ABSTRACTION or {}).get("phase","") or "").upper()=="CONVERSION_PATH_CAPACITY" and os.getenv("ORIGIN_CONVERSION_PATH_HYPOTHESIS","none").strip().lower() in ("throughput_match","throughput_with_slack") and not land_realizable_ok):
    ...
    realizable_received=abstraction_phase=="CONVERSION_PATH_CAPACITY"
    hypothesis_mode=os.getenv("ORIGIN_CONVERSION_PATH_HYPOTHESIS","none").strip().lower()
