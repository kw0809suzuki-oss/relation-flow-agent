#!/usr/bin/env python3
"""Fresh-12 paired evaluation: Full v1 vs Whole v2.4 wider-edge sparse."""
...
import full_strong_origin_v2_4_wider_edge as candidate_agent

OPPONENT = "opponents/seyamalam_v21.py"
CASES = tuple((seed, (seed - 3774) % 2) for seed in range(3774, 3786))
...
def main():
    ...
    totals = Counter()
    for row in rows:
        for key, value in row["candidate_telemetry"].get("whole_v2_4", {}).items():
    ...
    summary = {
        ...
        "whole_v2_4_totals": dict(totals),
    }

    result = {
        "schema": "kaggriculture.full-v2-4-wider-edge-pair.v1",
        "goal": "strengthen the current candidate one step while keeping external score-oriented observation",
        "baseline": "full_strong_origin_v1.py",
        "candidate": "full_strong_origin_v2_4_wider_edge.py",
        "cases": rows,
        "summary": summary,
        "runtime_inputs_excluded": ["seed", "paired_difference", "terminal_reward", "future_state"],
        "baseline_mutated": False,
        "causal_attribution": False,
        "evaluation_policy": "short-loop score update; use result for design judgment without assigning excess meaning",
    }
    Path("full_v2_4_wider_edge_pair_result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n")
    print("FULL_V2_4_WIDER_EDGE_PAIR " + json.dumps(summary, separators=(",", ":")))
