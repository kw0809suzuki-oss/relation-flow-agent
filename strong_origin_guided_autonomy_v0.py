...
def agent(obs):
    ...
    if strategy_name not in ("LIQUID","ENDGAME") and land_cost and remaining_days>=9 and occupancy>=strategy["occupancy_target"] and projected_cash-land_cost>=reserve and len(market)<10:
        if not (str((_FLOW_ABSTRACTION or {}).get("phase","") or "").upper()=="GUIDED_CONVERSION_PATH" and os.getenv("ORIGIN_GUIDANCE_MODE","none").strip().lower() in ("throughput_match","throughput_with_slack") and not land_realizable_ok):
    ...
    realizable_received=abstraction_phase=="GUIDED_CONVERSION_PATH"
    hypothesis_mode=os.getenv("ORIGIN_GUIDANCE_MODE","none").strip().lower()
    model_guidance_choice=hypothesis_mode
    model_guidance_reason="fixed_reference_or_control"
    if realizable_received and hypothesis_mode=="guided_autonomy":
        ...
        if remaining_days <= 5:
            ...
            model_guidance_reason="behind_competitor_prioritize_full_effective_throughput"
            ...
            model_guidance_reason="capacity_sufficient_preserve_slack"
